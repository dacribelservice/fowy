...
6.  **Regla de Viaje Único (RPC Consolidados)**: Si una interfaz requiere cargar información de más de dos tablas independientes de Supabase (ej: negocio, banners, categorías de menú), consolidar las consultas en una sola llamada RPC (PostgreSQL) para minimizar los viajes de red (RTT) desde el navegador.
