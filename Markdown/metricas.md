...
- [ ] **1.1. Acceder a la consola SQL de Supabase:** Ir a la sección "SQL Editor".
- [ ] **1.2. Crear la función RPC consolidada (`get_admin_rankings`):** Ejecutar el siguiente script SQL para crear la función que agrupa y cuenta las visitas y los pedidos, retornando el Top 10 de ambas métricas en formato JSON de forma altamente eficiente:
  ```sql
  CREATE OR REPLACE FUNCTION get_admin_rankings()
  RETURNS json AS $$
  DECLARE
    top_visits json;
    top_clicks json;
  BEGIN
    -- Top 10 Negocios por Visitas
    SELECT json_agg(t) INTO top_visits FROM (
      SELECT b.id, b.name, COUNT(v.id) as count
      FROM businesses b
      LEFT JOIN analytics_visits v ON b.id = v.business_id
      GROUP BY b.id, b.name
      ORDER BY count DESC
      LIMIT 10
    ) t;

    -- Top 10 Negocios por Clics de WhatsApp (Pedidos)
    SELECT json_agg(t) INTO top_clicks FROM (
    ...
    RETURN json_build_object(
      'top_visits', COALESCE(top_visits, '[]'::json),
      'top_clicks', COALESCE(top_clicks, '[]'::json)
    );
  END;
  $$ LANGUAGE plpgsql SECURITY DEFINER;
  ```
- [ ] **1.3. Probar el RPC:** Verificar directamente en el SQL Editor o en las pruebas de red que la respuesta entrega un JSON válido con `top_visits` y `top_clicks`.

## 📡 Fase 2: Orquestación de Datos (Frontend Orchestrator)

- [ ] **2.1. Modificar `src/app/admin/dashboard/page.tsx`:**
...
- [ ] **2.2. Fetching de Datos (Viaje Único):**
...
- [ ] **3.1. Crear archivo `src/components/admin/dashboard/DashboardRankingsList.tsx`:**
  - Inicializar este componente funcional para evitar exceder las 250 líneas en el archivo principal de la gráfica.
- [ ] **3.2. Desarrollar el diseño Premium (UI/UX):**
...
- [ ] **4.4. Renderizado Condicional de la Gráfica/Ranking:**
